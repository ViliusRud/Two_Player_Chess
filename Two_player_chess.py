...
p.display.set_caption('Two-Player Chess')
...
class Board:
    ...
    def promote_pawn(self, piece_type):
        ...
        if piece_type == 'queen':
            self.grid[pos[0]][pos[1]] = Queen(color, pos)
        elif piece_type == 'rook':
            self.grid[pos[0]][pos[1]] = Rook(color, pos)
        elif piece_type == 'bishop':
            self.grid[pos[0]][pos[1]] = Bishop(color, pos)
        elif piece_type == 'knight':
            self.grid[pos[0]][pos[1]] = Knight(color, pos)
    ...
    def draw(self, surface):
        ...
        if self.promoting_pawn:
            ...
            for i, option in enumerate(options):
                pygame.draw.rect(surface, WHITE, (i * SQUARE_SIZE, WINDOW_SIZE[1] - SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))
                text = font.render(option[0].upper(), True, BLACK)
                text_rect = text.get_rect(center=(i * SQUARE_SIZE + SQUARE_SIZE // 2, WINDOW_SIZE[1] - SQUARE_SIZE // 2))
...
class ChessGame:
    ...
    def handle_click(self, pos):
        if self.board.promoting_pawn:
            ...
            if 0 <= col < 4 and pos[1] > WINDOW_SIZE[1] - SQUARE_SIZE:
                options = ['queen', 'rook', 'bishop', 'knight']
                self.board.promote_pawn(options[col])
            return
        ...
        if 0 <= row < 8 and 0 <= col < 8:
            ...
            if self.board.selected_piece is None:
                ...
                if clicked_piece and clicked_piece.color == self.board.current_turn:
                    ...
                    if not self.board.move_piece(position):
